...
const MoodPicker: React.FC<MoodPickerProps> = ({ value, onChange }) => {
  const moods = [
    ...
    },
    {
      ...
      label: "Awful",
      icon: <Frown className="h-6 w-6 transform rotate-180" />,
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        {moods.map((mood) => (
          <button
            ...
            className={cn(
              "flex flex-col items-center p-3 rounded-lg transition-all duration-300 transform hover:scale-105",
              mood.color,
              value === mood.value ? 
                cn("ring-2 ring-offset-2 scale-105", mood.activeColor) : 
                "bg-muted/50"
            )}
          >
